...
namespace GitAlert.Model
{
    public sealed class RepositoryBrowser : IDisposable
    {
        ...
        {
            var ancestors = _repository.Commits.QueryBy(new CommitFilter { SortBy = CommitSortStrategies.Topological | CommitSortStrategies.Reverse }).ToList();
        ...
        private Commit GetCommitByName(string name, out string referenceName)
        {
            ...
            referenceName = tag.CanonicalName;
        ...
        private RepositoryCommitModel ToModel(Commit commit, bool withDiff = false)//, Tuple<bool, string, string> linkify)
        {
            ...
            var tags = _repository.Tags.Where(o => o.Target.Sha == commit.Sha).Select(o => o.CanonicalName).ToList();
